...
\*************************************************************************************************/
.linkform 'GP_TabWorkTimeT12_EM_2' prototype is 'TabT13' // Это клон TabWorkTimeT12.frm отличие только в RunInterface(iEMTabWorkTime, sPodr, dBeg, dEnd,1);
.Group 'Т-13' 
.nameinlist'*ЭМ.Табель учета рабочего времени Т-12 НПО Энергомаш для "Г","ГЧ"'
.F 'NUL'
.var
  marker: longint
  sPodr : string;
...
.}
.begin
   RunInterface(iEMTabWorkTime, sPodr, dBeg, dEnd,1);
   s.delete all tmpTabWorkTime;
end.
.endform
