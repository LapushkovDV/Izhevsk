...
FROM OPENQUERY([SCUD],  '
 ...
 ---
 ,max(re_o2.areas_id) as areas_id2
 ,max(ip.ip_addr) ip_addr
 ,re_o2.time_ev+45 as time_ev21
 ---
 from REG_EVENTS re
...
left outer join  REG_EVENTS re_o2 on 
  re_o2.staff_id=re.staff_id --and re_o2.type_pass=1
and (re_o2.areas_id=125937 or re_o2.areas_id =1318780) --and (re_o2.areas_id<>re.areas_id)
and re_o2.date_ev=re.date_ev and re_o2.time_ev>=re.time_ev-1
where re.date_ev>=CURRENT_DATE-10  
--and re.type_pass=1 08:52:36 08:52:36 125937
and (re.areas_id =1 )
----
...
and ((re_o2.id_reg is null) or re_o2.time_ev=
 (select min(re_o1.time_ev) as time_ev from REG_EVENTS re_o1
  where  (re_o1.areas_id=125937 or re_o1.areas_id=1318780 )
  --and re_o1.areas_id=re_o2.areas_id
  and re_o1.staff_id=re.staff_id 
  --and (re_o1.areas_id<>re.areas_id)
  and re_o1.date_ev=re.date_ev and re_o1.time_ev>=re.time_ev
  group by re_o1.staff_id --multiply,re_o1.areas_id
 )) 
